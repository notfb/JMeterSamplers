...
// TODO: copied from jmeter-trunk - create a maven dep for this ...
...
public abstract class JMeterTestCase extends TestCase {
    ...
    public JMeterTestCase(final String name) {
        super(name);
    }
    ...
    static {
        if (JMeterUtils.getJMeterProperties() == null) {
            ...
            final JMeterUtils jmu = new JMeterUtils();
            try {
    ...
    private static void logprop(final String prop) {
        System.out.println(prop + "=" + System.getProperty(prop));
    }

    // Helper method to find a file
    protected static File findTestFile(final String file) {
    ...
    protected void checkInvalidParameterCounts(final AbstractFunction func,
                                               final int minimum) throws Exception {
        final Collection parms = new LinkedList();
        for (int c = 0; c < minimum; c++) {
            try {
                func.setParameters(parms);
                fail("Should have generated InvalidVariableException for "
                        + parms.size() + " parameters");
            } catch (final InvalidVariableException ignored) {
            }
            parms.add("");
        }
        func.setParameters(parms);
    ...
    protected void checkInvalidParameterCounts(final AbstractFunction func,
                                               final int min, final int max) throws Exception {
        final Collection parms = new LinkedList();
